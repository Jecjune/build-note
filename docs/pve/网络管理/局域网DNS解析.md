建议[新开个lxc](远程连接.md#lxc)容器另外作为DNS服务器以及局域网反向代理，完成容器建立后先进行[宝塔面板的安装](../../ESC系列/网络配置/宝塔面板的安装.md)再进行以下操作。
...
首先直接关掉系统默认的DNS服务，否则53端口会被占用。（如果想保留`systemd-resolved`可以查看[解决53端口占用问题](解决53端口占用问题.md)，虽然我感觉没必要，因为后续DNS会由dnsmasq接管。）
```bash
...
## 编辑配置文件
编辑 dnsmasq 的配置文件 /etc/dnsmasq.conf 。这个文件包含大量的选项注释，具体解析请看[Dnsmasq配置文件示例](Dnsmasq配置文件示例.md)
...
接下来可以继续完成[局域网反向代理](局域网反向代理.md)，实现网址自动转入局域网服务
...
**到此DNS解析教程已结束，下面是bind9的失败尝试，仅进行记录。bind9在使用过程中不知道为什么被拒绝访问或者无法提示递归不可用，而且不太稳定，弃用了**


采用pve下lxc容器运行dns服务器，本文转载自：[Docker搭建DNS服务器 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/615495383)

先[新开个lxc](远程连接.md#lxc)并安装docker
...
如果建立不成功,查看日志和输出,如果是53端口占用,请看[解决53端口占用问题](解决53端口占用问题.md)，然后重新建立即可。
...
![局域网DNS1.png](../picture/局域网DNS1.png)
更改语言，选择简体中文-UTF8，记得刷新网页:
![局域网DNS2.png](../picture/局域网DNS2.png)
修改密码root密码：
![局域网DNS9.png](../picture/局域网DNS9.png)

设定解析默认值  
![局域网DNS2.png](../picture/局域网DNS3.png)

创建新的主区域-正向映射  
![局域网DNS4.png](../picture/局域网DNS4.png)

![局域网DNS5.png](../picture/局域网DNS5.png)
域名写要解析的域名，如：dawalker.top
email随便写写就好
主服务器指向自己就可以了(localhost)，因为只做本地解析
然后为新建的主区域添加ip映射

![局域网DNS6.png](../picture/局域网DNS6.png)
![局域网DNS7.png](../picture/局域网DNS7.png)
