...
### 修改开机界面的URL内容
文件位置为，修改开机时显示的网址，这样在忘记网址的时候可以通过该信息的输出获取：
```php
nano /etc/issue
```
...
ip link show
```
![网卡驱动1.png](../picture/网卡驱动1.png)
查看网卡驱动信息:
```php
ethtool -i enp4s0
```
![网卡驱动2.png](../picture/网卡驱动2.png)

查看网卡厂商信息:
```php
lspci | grep -i ethernet
```
![网卡驱动3.png](../picture/网卡驱动3.png)
...
#### 2.1 local的调整
建议local保留大小为30~40G，如果对磁盘的占用没有太离谱，强烈建议不要进行调整。
具体操作方法：[LV大小的调整](如何调整LVM相关PV、VG和LV大小.md#lv)
