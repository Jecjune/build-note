...
***直通即将整个硬盘的控制权交给该虚拟机，之后将只有该虚拟机能够使用，如果你的硬盘需要在多个地方都使用的话，或者已经在使用了，严禁执行直通操作***
...
***值得注意的是，按照上述方式载入的配置，当硬盘分区被重置的时候，该分区的uuid将失效，这会导致机器在开机过程中无法完全加载grub项，即进入‘emergency mode’，可以通过指定选项解决该问题，具体细节请看[[fatab文件概述]]。***

回到数据中心-储存-添加-目录：
![新增硬盘3.png](../picture/新增硬盘3.png)
